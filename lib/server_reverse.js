...
wst_server_reverse.prototype.start = function(port, hostAddress) {

  if (https_flag == "true")
    logger.info("[SYSTEM] - WS Reverse Tunnel Server starting on: wss://"+ hostAddress +":" + port + " - CERT: \n" + this.s4t_cert);
  else
    logger.info("[SYSTEM] - WS Reverse Tunnel Server starting on: ws://"+ hostAddress + ":" + port);

  //Activate HTTP/S server
  this.httpServer.listen(port, hostAddress, function() {
  ...
  this.wsServerForControll.on('request', (function(_this){
    return function(request){
      ...
      if (uri.query.dst != undefined){
        ...
        request.tcpServer.listen(portTcp, hostAddress);
        logger.info("[SYSTEM] --> TCP server is listening on host " + hostAddress + ", port " + portTcp);
